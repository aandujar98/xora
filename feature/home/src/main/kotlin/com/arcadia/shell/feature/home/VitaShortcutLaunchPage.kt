        ...
        BoxWithConstraints(
            ...
            if (revealPlate && page != null) {
                ...
                Box(modifier = Modifier.fillMaxSize()) {
                    ...
                    // Trophies and who else is in this title, in the sheet's bottom-right corner.
                    page.status?.let { status ->
                        val (insetX, insetY) = vitaLaunchStatusInset(unit)
                        VitaLaunchStatusPanel(
                            status = status,
                            unit = unit,
                            modifier = Modifier
                                .align(Alignment.BottomEnd)
                                .padding(end = insetX, bottom = insetY),
                        )
                    }
                }
                }
